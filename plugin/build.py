...
class Macro(object):
    ...
    def __post_init__(self):
        ...
        assert isinstance(self.commands, list), "macro {} has no commands".format(self.name)
...
class MenuItem(object):
    ...
    def __post_init__(
        ...
        if self.menu:
            ...
            self.menu_id = "".join([c for c in self.name if c.isalnum() or c in ["-", "_"]]).lower()
...
class Config(object):
    def __init__(
        ...
        self.custom_variables = [CustomVariable(v["name"], v["value"]) for v in custom_variables]

        macros = [
            {"name": "show-wk-menu", "commands": ["#{@wk_cmd_show}"]},
            {"name": "show-wk-menu-root", "commands": ["#{@wk_cmd_show} #{@wk_menu_root}"]},
        ] + macros
        self.macros = [
            Macro(self.command_alias_start_index + i, name=m["name"], commands=m["commands"])
    ...
    def register_menu(self, name: str, id: str, items: List[dict]):
        ...
        for item in items:
            ...
            if item.key:
                ...
                ), "keybinding {} was already registered in menu {}".format(item.key, name)
